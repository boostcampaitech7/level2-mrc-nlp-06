...
config_name="./config/extraction_config.json"
...
if [ "$TYPE" = "abs" ]; then
    ...
    if [ "$DO_EVAL" = true ]; then
        echo "Evaluation을 수행합니다..."
        python ./src/run_extraction.py --config "$config_name" --do_train --do_eval
    else
        echo "Evaluation을 건너뜁니다."
        python ./src/run_extraction.py --config "$config_name" --do_train
    fi
else
