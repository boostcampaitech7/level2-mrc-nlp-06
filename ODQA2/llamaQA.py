...
class LlamaQA:
    ...
    def load_model(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            self.args.model_name, quantization_config=bnb_config, device_map="auto"
        )
    ...
    def convert_squad_sample_to_llama_conversation(self, sample):
        ...
        sample_conversation = self.tokenizer.apply_chat_template(
    ...
    def predict(self, datasets, save_path):
        ...
        # 데이터 형태 변환 및 추론
        conversation_datasets = datasets.map(
            self.convert_squad_sample_to_llama_conversation
        )
        conversation_datasets = conversation_datasets.map(
            self.get_base_and_tuned_bulk_predictions, batched=True, batch_size=5
        )
