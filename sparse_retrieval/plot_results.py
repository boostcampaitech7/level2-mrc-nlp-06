...
def plot_results(csv_path: str, output_dir: str = './plots'):
    ...
    sns.lineplot(data=df, x='topk', y='hit@k', hue='embedding_method', marker='o', ax=axes[0])
    axes[0].set_title('Hit@k vs Topk')
    axes[0].set_xlabel('Top-K')
    axes[0].set_ylabel('Hit@k')

    sns.lineplot(data=df, x='topk', y='mrr@k', hue='embedding_method', marker='o', ax=axes[1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_path', type=str, default='test_sparse_embedding.csv', help="Path to the results CSV file")
