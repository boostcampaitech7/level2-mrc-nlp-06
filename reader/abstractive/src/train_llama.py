...
def convert_squad_sample_to_llama_conversation(
    sample, tokenizer, instruction_prompt_template
):
    ...
    messages = [
        {
            "role": "system",
            "content": instruction_prompt_template.format(context=context),
        },
...
def get_base_and_tuned_bulk_predictions(pipe, samples):
    ...
    responses = pipe(
        bulk_messages, max_new_tokens=64, batch_size=len(samples), do_sample=False
    )
...
def main():
    ...
    logger.info(
        f"{len(train_dataset)} data are used for training."
    )  # 데이터셋 크기 출력
    ...
    conversation_training_samples = train_dataset.map(
        lambda sample: convert_squad_sample_to_llama_conversation(
            sample, tokenizer, instruction_prompt_template
        )
    )
    conversation_validation_samples = eval_dataset.map(
        lambda sample: convert_squad_sample_to_llama_conversation(
            sample, tokenizer, instruction_prompt_template
        )
    )
    ...
    model.config.use_cache = (
        False  # 학습 중이고, output이 변경될 것으로 예상되므로 False
    )
    ...
    trainer.save_model(
        args.output_dir
    )  # 학습된 weight 저장: adapter weight만 학습했으므로 이 adapter만 저장됨
    ...
    conversation_validation_samples = conversation_validation_samples.map(
        lambda samples: get_base_and_tuned_bulk_predictions(
            pipe=model_pipe, samples=samples
        ),
        batched=True,
        batch_size=5,
    )
    ...
    trained_validation_bert_score = bertscore.compute(
    ...
    tuned_averages = {
        key: sum(trained_validation_bert_score[key])
        / len(trained_validation_bert_score[key])
    ...
    ex_predictions = [
        {"id": sample["id"], "prediction_text": sample["predicted_answer"]}
        for sample in conversation_validation_samples
    ]
    ex_references = [
        {
    ...
    tuned_exact_match_score = em_f1_score.compute(
        predictions=ex_predictions, references=ex_references
    )
