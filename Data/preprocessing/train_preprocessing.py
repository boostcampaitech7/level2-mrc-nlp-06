...
def check_markdown(df, column="text"):
    ...
    for text in df[column]:
        ...
        # 이탤릭체 개수
        italic_count = (
            len(re.findall(r"\*(.*?)\*", text)) - bold_count
        )  # 볼드체가 이탤릭체와 겹침
        # 코드 블록 개수
...
def combine_answer(df):
    return df[["answer_start", "answer_text"]].apply(
        lambda x: {"answer_start": [(x["answer_start"])], "text": [x["answer_text"]]},
        axis=1,
    )
...
def preprocessing_main(df, name, newline=False, mark=False, lang=False):
    ...
    df["answer_word_idx"] = df.apply(
        lambda row: get_answer_word_idx(row["context"], row["answer_text"]), axis=1
    )

    # answer_word_true_idx라는 컬럼에 answer_word_idx를 기준으로 정답 value의 인덱스를 저장
    df["answer_word_true_idx"] = df.apply(
        lambda row: get_answer_idx(row["answer_start"], row["answer_word_idx"]), axis=1
    )
    ...
    df["answer_word_idx_after"] = df.apply(
        lambda row: get_answer_word_idx(row["context"], row["answer_text"]), axis=1
    )

    # answer_word_idx_after에서 answer_word_true_idx번재의 인덱스 값을 answer_start에 덮어서 저장
    df["answer_start"] = df.apply(
        lambda row: get_after_answer_start(
            row["answer_word_idx_after"], row["answer_word_true_idx"]
        ),
        axis=1,
    )
    ...
    # 필요 없는 컬럼 삭제
    df = df.drop(
        columns=["answer_word_idx", "answer_word_true_idx", "answer_word_idx_after"]
    )
...
def main():
    ...
    for i in range(len(version_para)):
        ...
        train_df = preprocessing_main(
            train_df,
            "train",
            newline=version_para[i][0],
            mark=version_para[i][1],
            lang=version_para[i][2],
        )
        valid_df = preprocessing_main(
            valid_df,
            "valid",
            newline=version_para[i][0],
            mark=version_para[i][1],
        ...
        df.to_json(
            save_path + "/wikipedia_documents.json", orient="index", force_ascii=False
        )
