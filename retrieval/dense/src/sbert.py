...
class BiEncoder:
    ...
    def train(
        ...
        train_retrieval = train.remove_columns(
            [col for col in train.column_names if col not in ["question", "context"]]
        )
        train_retrieval = train_retrieval.rename_columns(
            {"question": "anchor", "context": "positive"}
        )
        valid_retrieval = valid.remove_columns(
            [col for col in valid.column_names if col not in ["question", "context"]]
        )
        valid_retrieval = valid_retrieval.rename_columns(
            {"question": "anchor", "context": "positive"}
        )
    ...
    def retrieve(self, data_type, stage, topk, save_path=None):
        ...
        hits = util.semantic_search(
            query_embeddings=que_embs, corpus_embeddings=doc_embs, top_k=topk
        )
        ...
        if stage == 1:  # Bi-Encoder만으로 retrieval 수행
            ...
            if (
                data_type == "train" or data_type == "valid"
            ):  # train, validation dataset
                result_df = result_df.drop(columns=["title", "__index_level_0__"])
                result_df["original_context"] = result_df["context"]

                for i, hit in tqdm(
                    enumerate(hits), desc=f"retrieve {data_type}", total=len(hits)
                ):
                    result_df.loc[i, "context"] = " ".join(
                        [documents[h["corpus_id"]] for h in hit]
                    )
            else:  # test dataset
            ...
            stage == 2
        ):  # Bi-Encoder에서 retrieve한 결과를 re-ranking할 수 있도록 하는 형태
            return hits
        else:
            print(
                f'{stage} for "stage" is not appropriate. "stage" should be either 1 or 2'
            )
...
class CrEncoder:
    ...
    def train(
        self,
        output_dir="./sbert-cross-encoder",
        epochs=5,
        batch_size=32,
        weight_decay=0.01,
    ):
        ...
        for i in tqdm(range(train.num_rows), desc="pre-train"):
            train_reranker.append(
                InputExample(texts=[train["question"][i], train["context"][i]], label=1)
            )
        train_dataloader = DataLoader(
            train_reranker, shuffle=True, batch_size=batch_size
        )
    ...
    def validate(self, data_type, topk):
        ...
        evaluator = CERerankingEvaluator(
            samples=valid_reranker, at_k=topk, name="ranker"
        )
        results = evaluator(self.model)
    ...
    def retrieve(
        self, data_type, topk, save_path=None
    ):  # Cross Encoder만으로 retrieval 수행
        documents = load_corpus(self.data_path)
        ...
        print(
            f"searching relevant documents: it might take a long time depending on the size of the corpus\n"
        )
        results = []
        ...
        if data_type == "train" or data_type == "valid":  # train, validation dataset
            result_df = pd.DataFrame(result_df).drop(
                columns=["title", "__index_level_0__"]
            )
            result_df["original_context"] = result_df["context"]
