...
from utils.utils_sparse_retrieval import (
    load_config,
    timer,
    evaluate,
    append_to_csv,
    append_to_csv_learned,
)
...
def main():
    ...
    wiki_unique_df = wiki_df.drop_duplicates(
    ...
    if args.embedding_method in ["tfidf", "bm25"]:
        ...
        if args.tokenizer_model_name == "mecab":
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                args.tokenizer_model_name, use_fast=True
            )
    ...
    if args.embedding_method in ["tfidf", "bm25"]:
        ...
        retrieved_df = learned_sparse_retriever.retrieve(
            eval_df, dense_metric_type=args.dense_metric_type, topk=args.topk
        )
    ...
    evaluation_file_name = (
        f"{sparse_path_name}/outputs/{args.embedding_method}_test_results.csv"
    )
    if args.embedding_method in ["tfidf", "bge-m3"]:
        ...
        append_to_csv_learned(
            evaluation_file_name, args, total_time, evaluation_results
        )
