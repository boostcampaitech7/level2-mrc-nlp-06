...
class HybridRetrieval:
    ...
    def set_dense_model(self):
        ...
        q_encoder = AutoModel.from_pretrained(model_name_or_path+"/q/").to(self.device)
        p_encoder = AutoModel.from_pretrained(model_name_or_path+"/p/").to(self.device)
            ...
            with timer("passage dense embedding time:"):
                with torch.no_grad():
                    for i, batch in enumerate(dataloader):
                        tokenized_contexts = tokenizer(batch, padding=True, truncation=True, return_tensors='pt')
    ...
    def get_passage_hybrid_embedding(self):
        ...
        if os.path.isfile(dimension_reducer_path):
            ...
            with timer("Build passage embedding"):
                ...
                self.dimension_reducer = TruncatedSVD(n_components=768, algorithm="arpack", random_state=seed)
                dimension_reducer = self.dimension_reducer
                ...
                with timer("query exhaustive search"):
                    ...
                    if len(doc_scores) == 1:
                        ...
                        for idx, example in enumerate(
                            ...
                            document_ids = [self.document_ids[pid] for pid in doc_indices[idx]] if self.document_ids is not None else None
    ...
    def get_relevant_doc(self, retriever_type = "sparse", queries: List[str] = None, k: Optional[int] = 1) -> Tuple[List, List]:
        ...
        if retriever_type in ["dense", "hybrid"]:
            with torch.no_grad():
                tokenized_queries = self.dense_tokenizer(queries, padding=True, truncation=True, return_tensors='pt').to(self.device)
